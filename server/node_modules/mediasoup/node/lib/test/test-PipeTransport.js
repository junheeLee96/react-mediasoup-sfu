...
afterEach(async () => {
    ...
    if (ctx.worker1?.subprocessClosed === false) {
        await new Promise(resolve => ctx.worker1?.on('subprocessclose', resolve));
    }
    if (ctx.worker2?.subprocessClosed === false) {
        await new Promise(resolve => ctx.worker2?.on('subprocessclose', resolve));
    }
});
...
test('producer.pause() and producer.resume() are transmitted to pipe Consumer', async () => {
    ...
    const promise1 = new Promise(resolve => videoConsumer.once('producerresume', resolve));
    await ctx.videoProducer.resume();
    await promise1;
    expect(videoConsumer.producerPaused).toBe(false);
    expect(videoConsumer.paused).toBe(false);
    const promise2 = new Promise(resolve => videoConsumer.once('producerpause', resolve));
    await ctx.videoProducer.pause();
    await promise2;
    expect(videoConsumer.producerPaused).toBe(true);
    expect(videoConsumer.paused).toBe(false);
